...
Page({
  ...
  async loadCartItems() {
    const cartIds = wx.getStorageSync('checkoutCartIds') || []
    if (cartIds.length === 0) {
  ...
  onSelectAddress(e) {
    this.setData({ selectedAddressId: Number(e.currentTarget.dataset.id) })
  },
    ...
    try {
      ...
      wx.requestPayment({
        ...
        fail: (err) => {
          ...
          wx.showToast({ title: '待支付，可在订单中继续付款', icon: 'none', duration: 2000 })
          setTimeout(() => {
            wx.switchTab({ url: '/pages/order/index' })
          }, 2000)
        }
